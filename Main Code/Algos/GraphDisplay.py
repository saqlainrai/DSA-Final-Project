...
def roundTrip(city_graph, vertices):
    ...
    print("Shortest Distance 1: ",shortest_Distance1,shortest_path1)
    shortest_Distance2,shortest_path2=shortest_path_to_visit_all(city_graph, vertices)
    print("Shortest Distance 1: ",shortest_Distance2,shortest_path2)
    shortest_Distance1 += shortest_Distance2
    shortest_path1.extend(shortest_path2)
    return shortest_Distance1,shortest_path1

def selfRoundTrip(city_graph, vertices):
    start = 0
    distance = 0
    for i in range(len(vertices)):
        n, d = findSoleCity(city_graph, vertices, start)
        vertices = vertices.remove(vertices[start])
        start = n
        distance += d 
    print("Final Distance: ", distance)

def findSoleCity(city_graph, vertices, cmp):
        startVertex = vertices[cmp]
        nearestVertexIndex = 1
        Shortdistance, _ = FindShortestPath(city_graph, startVertex, vertices[nearestVertexIndex])
        for i in range(1,len(vertices)):
            distance, path = FindShortestPath(city_graph, startVertex, vertices[i])
...
def main():
    ...
    g.add_connection("a", "c", 6)
    g.add_connection("a", "f", 1)
    g.add_connection("b", "f", 3)
    g.add_connection("c", "d", 15)
    g.add_connection("c", "e", 12)
    g.add_connection("d", "e", 9)
    g.add_connection("e", "f", 7)
    list_of_vertices = ["a", "b", "c", "d", "e", "f"]
    selfRoundTrip(g, list_of_vertices)
